...
- **A `Degraded` app-of-apps with every resource Synced and healthy is usually a CronJob, and the failed Job is already gone**: ArgoCD's CronJob health check reports `CronJob has not completed its last execution successfully` whenever `lastSuccessfulTime` is older than `lastScheduleTime`, which makes the whole root app Degraded. `ttlSecondsAfterFinished: 120` deletes the failed Job and its events minutes later, so `kubectl get jobs` and the event log both come back clean and the cause looks invisible — compare the *date* of `lastScheduleTime` vs `lastSuccessfulTime` across all CronJobs (`kubectl get cronjob -A`) and read the pod's logs from Loki, which outlive the Job. Do not try to infer the culprit from `Application.status.resources`: ArgoCD 3.x does not persist per-resource `health` there (every entry is nil), so the custom root-app Lua's `resource.health` branch is dead code, and the app's own `status.health` carries no message field to read. The status is genuinely recomputed each cycle, so a hard refresh, a controller restart, and patching `status.health` all fail to clear it — only a subsequent successful *scheduled* run does (a `kubectl create job --from=cronjob/...` gets no owner reference, so it never updates `lastSuccessfulTime`).
