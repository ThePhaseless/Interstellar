...
resource "bitwarden-secrets_secret" "tailscale_oauth_client_id" {
  ...
  note       = "Tailnet management OAuth client ID (Main Key)."
  lifecycle {
...
resource "bitwarden-secrets_secret" "tailscale_oauth_secret" {
  ...
  value      = ""
  project_id = local.bitwarden_project_id
  note       = "Tailnet management OAuth client secret (Main Key)."
  lifecycle {
    ignore_changes = [value]
    postcondition {
      condition     = self.value != ""
...
resource "bitwarden-secrets_secret" "tailscale_tailnet" {
  ...
  value      = ""
  project_id = local.bitwarden_project_id
  note       = "Tailnet name (e.g. user.ts.net)."
  lifecycle {
    ignore_changes = [value]
    postcondition {
      condition     = self.value != ""
...
resource "bitwarden-secrets_secret" "cloudflare_api_token" {
  ...
  value      = ""
  project_id = local.bitwarden_project_id
  note       = "Cloudflare API token with DNS edit permissions."
  lifecycle {
    ignore_changes = [value]
    postcondition {
      condition     = self.value != ""
...
resource "bitwarden-secrets_secret" "oci_config" {
  ...
  value      = ""
  project_id = local.bitwarden_project_id
  note       = "OCI SDK configuration file content."
  lifecycle {
    ignore_changes = [value]
    postcondition {
      condition     = self.value != ""
...
resource "bitwarden-secrets_secret" "tf_state_bucket" {
  ...
  value      = "tf-state"
  project_id = local.bitwarden_project_id
  note       = "Name of the OCI bucket for Terraform state."
  lifecycle {
    ignore_changes = [value]
    postcondition {
      condition     = self.value != ""
...
resource "bitwarden-secrets_secret" "proxmox_user" {
  ...
  value      = ""
  project_id = local.bitwarden_project_id
  note       = "Proxmox user for API authentication."
  lifecycle {
    ignore_changes = [value]
    postcondition {
      condition     = self.value != ""
...
resource "bitwarden-secrets_secret" "gh_app_id" {
  ...
  value      = "placeholder"
  project_id = local.bitwarden_project_id
  note       = "GitHub App ID for CI authentication."
  lifecycle {
    ignore_changes = [value]
  }
}
...
resource "bitwarden-secrets_secret" "gh_app_private_key" {
  ...
  value      = "placeholder"
  project_id = local.bitwarden_project_id
  note       = "GitHub App private key (PEM) for CI authentication."
  lifecycle {
    ignore_changes = [value]
  }
}
...
resource "bitwarden-secrets_secret" "talosconfig" {
  ...
  note       = "Talosconfig for talosctl CLI access"
...
resource "bitwarden-secrets_secret" "oci_objectstorage_namespace" {
  ...
  note       = "OCI Object Storage namespace for Terraform state bucket"
  project_id = local.bitwarden_generated_project_id
}
...
resource "bitwarden-secrets_secret" "cluster_vip" {
  ...
  note       = "Talos/Kubernetes control-plane virtual IP"
  project_id = local.bitwarden_generated_project_id
}
