...
- **Every tailscaled behind one consumer NAT must get its own `--port`, or the node that loses the mapping thrashes and takes its own DNS down with it**: all three Talos nodes defaulted to UDP 41641, and the Funbox can map only one of them. After a router event on 2026-09-21 talos-3 lost its mapping, fell back to a DERP relay and rebound its source port every few seconds; that churn exhausted the router's UDP mappings for `192.168.1.112` alone, dropping 70-90% of that node's DNS replies (both `1.1.1.1` and `8.8.8.8`) while talos-1/2 measured 0% loss on the same bridge. It presents as etcd health-check timeouts, Longhorn replica i/o timeouts and ~hourly `Ready` flaps — nothing points at Tailscale. `TS_EXTRA_ARGS` now pins `--port=${40000 + vmid}` (vmid-keyed so adding a node never renumbers the rest). Diagnose with `tailscale status`: a node showing `relay "..."` instead of `direct` is the wedged one, and Talos will not rescue it — extension services carry no health check (`HEALTH ?` in `talosctl services`), so a wedged tailscaled runs forever until restarted by hand.
