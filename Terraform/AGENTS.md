...
- **Jellyfin library options are now typed**: use the `library_options` block instead of `library_options_json`; server-owned fields are left unconfigured and drift is suppressed by Optional+Computed attributes.
- **SSO-Auth plugin configuration is now typed**: use `jellyfin_sso_plugin_configuration` with `oid_configs`/`saml_configs` maps instead of `jellyfin_plugin_configuration` raw JSON; `canonical_links` and other server-managed fields are omitted from config and suppressed by Optional+Computed attributes.
...
- **Default Kubernetes provider endpoint uses `talos-2`, not `talos-1`**: `talos-1` is the GPU node and is recreated more often during hardware/GPU experiments; its Tailscale identity does not survive reinstallation. The provider default falls back to `talos-2.fold-hen.ts.net`; use `var.kubernetes_api_host` only to override to a currently reachable node, and do not hardcode `talos-1` in CI.
