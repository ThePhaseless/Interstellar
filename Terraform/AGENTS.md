...
- **`talos_machine` (provider 0.12.0) reports every successful OS upgrade as failed after a 15-minute wait**: on the v1.13.9 → v1.14.1 rollout each node rebooted into the new version, committed it (`removing fallback entry` in dmesg) and was Ready within a minute, yet the provider's reboot tracker never saw its expected event and returned `waiting for node after reboot: reboot sequence completed; timeout`. The deferred uncordon still ran and Terraform moved on to the next node, so the rollout itself is correct but takes ~20 minutes per node and ends with exit 1 — a Renovate-driven CI apply will go red. State keeps the new `image`; the config re-apply the error cut short (install image tag) shows up as an in-place `talos_machine` change on the next plan and applies without a reboot. Cause not pinned yet; suspect a half-open event stream across the reboot.
