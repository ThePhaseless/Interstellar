...
- **CI hostname resolution is not guaranteed by MagicDNS alone**: Use Talos hostnames for provider access, but on GitHub runners populate `/etc/hosts` from `tailscale status --json` peer `DNSName` values before Terraform or kubectl calls.
