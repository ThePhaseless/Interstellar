...
- **Authentik's `webhook_slack` transport needs Discord's `/slack` URL variant**: Discord returns 400 on a Slack-shaped payload posted to the bare webhook, and Authentik surfaces it only as a failed background task. Append `/slack` to the `discord-webhook-url` secret when building `webhook_url`. Test delivery with `POST /api/v3/events/transports/<uuid>/test/` (a 200 body of `{"messages":["200","ok"]}` is Discord's own status echoed back), and confirm a rule end-to-end in the worker log — `authentik.events.tasks.notification_transport` finishing with `exc: null`. `/api/v3/events/notifications/` only lists the *calling* user's notifications, so it looks empty even when the rule fired for someone else.
