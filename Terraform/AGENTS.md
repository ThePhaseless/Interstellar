...
- **A node whose tailscaled cannot traverse the NAT poisons its own DNS, and it presents as etcd timeouts and `Ready` flaps, never as a Tailscale fault**: on 2026-09-21 talos-3 began logging `netcheck: UDP is blocked` and `NetInfo{udp=false}`, fell back to a DERP relay, and retried endpoint discovery hard enough that 70-90% of that node's DNS replies were dropped — measured on `vmbr0`, where talos-1/2 sat at 0% against the same resolvers. The replies never reach the bridge, so the loss is upstream of the host: the router, not the VM. Restarting `ext-tailscale` clears it for ~10 minutes, which is the tell. Knock-on symptoms were etcd health-check timeouts (128/day vs 11 on peers), Longhorn replica i/o timeouts and 42 `Ready` flaps in 24h with zero on any prior day. Pinning a distinct `--port` per node (now done, keyed on vmid) is correct hygiene but **did not fix this** — the loss returned after the apply, so port collision was not the cause. Diagnose with `tailscale status`: the wedged node shows `relay "..."` rather than `direct`. Talos will not rescue it either — extension services carry no health check (`HEALTH ?` in `talosctl services`), so a wedged tailscaled runs until restarted by hand.
