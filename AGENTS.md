...
- **The Oracle VPS runs Tailscale with `--accept-dns=false` by design** (commit `4ba7c50`, security: VPS uses OCI DNS, never home AdGuard; also note `ansible.yaml` CI connects with `--accept-dns=false`). Consequence: MagicDNS is dead on the VPS (`100.100.100.100` does not answer), so `haproxy.cfg` **must not** resolve the Traefik backend via a MagicDNS name (`resolvers tailscale`/`traefik.fold-hen.ts.net`) — HAProxy fails at startup with `could not resolve address` and crash-loops, taking down all public access. The backend uses the **pinned tailnet IP `100.72.236.33`**, enforced by the `tailscale.com/tailnet-ip` annotation on the traefik Service (`Kubernetes/bootstrap/traefik/kustomization.yaml`). If you change that IP, update both places.
