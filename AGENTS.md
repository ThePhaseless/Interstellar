...
- **The Oracle VPS runs Tailscale with `--accept-dns=false` by design** (commit `4ba7c50`, security: VPS uses OCI DNS, never home AdGuard; also note `ansible.yaml` CI connects with `--accept-dns=false`). Consequence: MagicDNS is dead on the VPS (`100.100.100.100` does not answer), so `haproxy.cfg` **must not** use `resolvers tailscale` — HAProxy fails at startup with `could not resolve address 'traefik.fold-hen.ts.net'` and crash-loops, taking down all public access (this exact incident: 2026-07-24 commit broke it, outbound dead from ~07-26). The backend name resolves via libc/`/etc/hosts`, kept in sync by the **`traefik-ip-sync` systemd timer** on the VPS (`sync-traefik-ip.sh`, reads `tailscale status`, writes `/etc/hosts`, SIGHUPs HAProxy; runs every 60s; deployed by `setup-oracle.yaml`). The tailscale k8s operator cannot pin the traefik device IP (`tailscale.com/tailnet-ip` is not supported in operator v1.98.9 — verify before trusting it again).
